...
ASR、单条 TTS、有序 TTS 序列、克隆参考分析和声纹样本上传这五个原生异步提交入口都要求 8–128 字符的 `Idempotency-Key`。首次接受返回 `202`，同请求重放返回 `200`，同键更改输入返回 `409`。序列任务一次加载模型，最多处理 100 条有序文本，并为每条返回独立 WAV；实际限制以 `tts.sequence_jobs` 能力为准。
