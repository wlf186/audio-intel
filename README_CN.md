...
| **本地语音工作室** | Qwen3-TTS 0.6B/1.7B、预置音色、一次性或声纹库声音克隆（可选 3–30 秒声纹参考区间）、1.7B VoiceDesign，以及 WAV/FLAC/MP3 输出 |
| **文档语音合成** | EPUB、TXT、Markdown、文本 PDF、DOCX、XLSX、PPTX，结构／字数分段预览、导入复用与批量移除、检查点恢复及带专辑标签的流式 ZIP / 完整 MP3 下载 |
...
文档 TTS 支持 EPUB、TXT、Markdown、文本 PDF、DOCX、XLSX 和 PPTX，默认最多 500 万字符及 2000 段。上传后预览和选择章节，复用现有音色，按段持久化 MP3 并支持恢复；ZIP 和完整 MP3 按需流式下载。旧导入可管理、复用和手动删除，详见 [文档 TTS](docs/DOCUMENT_TTS.md)。

克隆参考默认最多使用样本开头 15 秒。声纹库样本可通过波形编辑器选择任意位置的 3–30 秒区间，按完整词边界同步截取音频与文字，并保存任务快照。详见[参考区间](docs/API.md#manual-voiceprint-reference-ranges)。

文档合成支持复用导入与跨页多选移除；结构模式仅在缺少可靠结构时使用备用目标字数。正文可切换“原文换行／连续阅读”，下载的分段和完整 MP3 携带同一任务的专辑标签。

TTS 对每个内部文本块执行异常生成保护，异常块最多额外重试 3 次，文档已完成段仍可复用。保护覆盖超长失控与无效波形，不能代替发音或听感检查；详见[行为与限制](docs/API.md#tts-generation-guard--语音生成保护)。
