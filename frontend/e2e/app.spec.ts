...
test('system worker state and heartbeat use Chinese local presentation',async({page})=>{
 ...
 await expect(page.locator('footer .footer-id')).toHaveText('非官方')
 const identity=page.locator('.project-identity')
 await expect(identity).toContainText('独立非官方项目')
 await expect(identity).toContainText('未获得 CD PROJEKT RED、R. Talsorian Games 或任何相关权利方的授权、赞助、认可或背书')
 ...
 await expect(page.locator('footer .footer-id')).toHaveText('UNOFFICIAL')
 await expect(identity).toContainText('Independent unofficial project')
 await expect(identity).toContainText('has not been authorized, sponsored, approved, or endorsed')
 await page.screenshot({path:'/tmp/audio-intel-project-identity-desktop.png',fullPage:false})
 await page.getByLabel('Interface language').selectOption('zh-CN')
 await page.setViewportSize({width:390,height:844})
 await expect(identity).toContainText('独立非官方项目')
 await expect(page.locator('footer')).not.toBeVisible()
 expect(await page.evaluate(()=>document.documentElement.scrollWidth)).toBeLessThanOrEqual(390)
 await page.screenshot({path:'/tmp/audio-intel-project-identity-mobile.png',fullPage:false})
 expect(errors).toEqual([])
})
