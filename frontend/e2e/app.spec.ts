import {expandAsrAdvanced,expandAsrSettings,showAsrResults} from './asr-helpers'
...
test('local API docs load offline and execute the health probe',async({page,baseURL})=>{
 const errors:string[]=[]
 ...
 await expect(firstTag.locator('.opblock-tag')).toBeVisible()
...
test('ordered TTS docs expose voiceprint requests and per-item results',async({page,baseURL})=>{
 const errors:string[]=[]
 const external:string[]=[]
 page.on('pageerror',error=>errors.push(error.message))
 page.on('console',message=>{if(message.type()==='error')errors.push(message.text())})
 ...
 await page.goto('/docs')
 await expect(page).toHaveTitle(/Sandevistan/)
 const recipe=page.locator('details').filter({has:page.locator('summary').filter({hasText:'Ordered TTS: submit, wait, and download'})})
 await recipe.locator('summary').click()
 await expect(recipe.locator('pre')).toContainText('Idempotency-Key: $SEQUENCE_KEY')
 ...
 await page.locator('.opblock-tag').filter({hasText:'TTS / 语音合成'}).click()
 const sequence=page.locator('.opblock').filter({has:page.locator('.opblock-summary-path[data-path="/api/v1/tts/sequence-jobs"]')})
 await sequence.locator('.opblock-summary').click()
 const requestExample=sequence.locator('.body-param__example')
 await expect(requestExample).toBeVisible()
 await sequence.locator('select:has(option[value="voiceprint"])').selectOption('voiceprint')
 await expect(requestExample).toContainText('voiceprint_sample_id')
 await requestExample.scrollIntoViewIfNeeded()
 expect(await page.evaluate(()=>document.documentElement.scrollWidth)).toBeLessThanOrEqual(1440)
 await page.screenshot({path:'/tmp/audio-intel-docs-sequence-desktop.png',fullPage:false})
 await page.setViewportSize({width:390,height:844})
 await sequence.locator('select:has(option[value="voiceprint"])').selectOption('podcast_turns')
 expect((await sequence.locator('select:has(option[value="voiceprint"])').boundingBox())!.height).toBeGreaterThanOrEqual(44)
 await expect(requestExample).toContainText('Vivian')
 expect(await page.evaluate(()=>document.documentElement.scrollWidth)).toBeLessThanOrEqual(390)
 await requestExample.scrollIntoViewIfNeeded()
 await page.screenshot({path:'/tmp/audio-intel-docs-sequence-mobile.png',fullPage:false})
 await page.locator('.opblock-tag').filter({hasText:'Jobs / 任务'}).click()
 const result=page.locator('.opblock').filter({has:page.locator('.opblock-summary-path[data-path="/api/v1/jobs/{job_id}/result"]')})
 await result.locator('.opblock-summary').click()
 await result.locator('select:has(option[value="tts_sequence"])').selectOption('tts_sequence')
 const resultExample=result.locator('pre').filter({hasText:'artifact_name'})
 await expect(resultExample).toContainText('item-0001.wav')
 await expect(resultExample).toContainText('contract_version')
 await resultExample.scrollIntoViewIfNeeded()
 expect((await resultExample.boundingBox())!.width).toBeGreaterThan(250)
 expect((await result.locator('select:has(option[value="tts_sequence"])').boundingBox())!.height).toBeGreaterThanOrEqual(44)
 expect(await page.evaluate(()=>document.documentElement.scrollWidth)).toBeLessThanOrEqual(390)
 await page.screenshot({path:'/tmp/audio-intel-docs-sequence-result-mobile.png',fullPage:false})
 expect(external).toEqual([])
 expect(errors).toEqual([])
})
...
test('Sandevistan-Audio branding and TTS transport render as local assets',async({page})=>{
 ...
 page.on('console',message=>{if(message.type()==='error')errors.push(message.text())})
 const now=new Date().toISOString()
 const job={id:'local-tts-preview',kind:'tts',state:'succeeded',stage:'completed',progress:1,display_name:'本地合成预览',created_at:now,updated_at:now,request:{compute_device:'cpu'},result:{duration:5,waveform:Array(240).fill(.4),format:'wav',speaker:'Vivian',artifacts:[{name:'speech.wav',mime_type:'audio/wav',size_bytes:160044}]}}
...
test('TTS exposes the 0.6B control boundary without sending unsupported instructions',async({page})=>{
 ...
 if(!await note.isVisible())await page.locator('.document-quality-help summary').click()
...
test('one-off clone reference model controls use the available width',async({page})=>{
 ...
 const expectStacked=async(width:number,height:number)=>{
  ...
  expect(deviceBox!.y).toBeGreaterThanOrEqual(modelBox!.y+modelBox!.height)
  expect(Math.abs(modelBox!.width-deviceBox!.width)).toBeLessThan(2)
  ...
  expect(hintBox!.width).toBeGreaterThanOrEqual(deviceBox!.width-2)
 }
 await expectStacked(1440,900)
 await panel.screenshot({path:'/tmp/audio-intel-clone-controls-desktop.png'})
 await expectStacked(1024,820)
 await page.setViewportSize({width:390,height:844})
 await panel.scrollIntoViewIfNeeded()
...
test('ASR model routing and remembered hotword selection are explicit',async({page})=>{
 ...
 await page.goto('/#asr')
 await expandAsrAdvanced(page)
 await page.getByLabel('ASR 模型').selectOption('qwen3-asr-1.7b')
 await expect(page.getByLabel('ASR 计算设备')).toHaveValue('cpu')
 await expect(page.getByText(/至少需要 7936 MiB 显存.*4096 MiB/).first()).toBeVisible()
 await expandAsrSettings(page)
 await page.getByRole('button',{name:'选择热词表',exact:true}).click()
 ...
 await expect(page.getByRole('dialog').getByRole('alert').filter({hasText:'当前热词选择已超出单次任务限制'})).toContainText(/3 个唯一热词.*上限 2 个/)
...
test('desktop submit actions remain visible and voiceprint model controls reflow cleanly',async({page})=>{
 ...
 const expectNoControlOverlap=async(scope:string)=>{const layout=await page.locator(scope).evaluate(element=>{const action=element.querySelector('.submission-actions')!.getBoundingClientRect();const controls=[...element.querySelectorAll('select,.acceleration-control')].map(control=>{const box=control.getBoundingClientRect();const clip=control.closest('.tts-settings-body,.asr-settings-body')?.getBoundingClientRect();return {label:control.getAttribute('aria-label')||control.className,top:Math.max(box.top,clip?.top??box.top),bottom:Math.min(box.bottom,clip?.bottom??box.bottom)}});return {action:{top:action.top,bottom:action.bottom},controls}});expect(layout.controls.filter(control=>control.bottom>control.top&&control.bottom>layout.action.top&&control.top<layout.action.bottom),JSON.stringify(layout)).toEqual([])}
 await page.setViewportSize({width:1440,height:900})
 await page.goto('/#asr')
 ...
 await page.setViewportSize({width:1024,height:820})
 await page.getByRole('button',{name:'生成语音'}).scrollIntoViewIfNeeded()
 await expectInsideMain('生成语音')
 await page.goto('/#voiceprints')
...
test('system worker state and heartbeat use Chinese local presentation',async({page})=>{
 ...
 await expect(page).toHaveTitle(/系统状态 · Sandevistan-Audio/)
 await expect(page.locator('.project-identity')).toHaveCount(0)
 await expect(page.locator('.mobile-project-identity')).not.toBeVisible()
 const footerNotice=page.getByRole('button',{name:'非官方',exact:true})
 const identityTooltip=page.locator('#project-identity-tooltip')
 await expect(footerNotice).toBeVisible()
 await footerNotice.hover()
 await expect(identityTooltip).toBeVisible()
 await expect(identityTooltip).toContainText('未获得 CD PROJEKT RED、R. Talsorian Games 或任何相关权利方的授权、赞助、认可或背书')
 const tooltipBox=await identityTooltip.boundingBox()
 const footerBox=await page.locator('footer').boundingBox()
 expect(tooltipBox!.x).toBeGreaterThanOrEqual(0)
 expect(tooltipBox!.x+tooltipBox!.width).toBeLessThanOrEqual(1440)
 expect(tooltipBox!.y+tooltipBox!.height).toBeLessThanOrEqual(footerBox!.y)
 await page.screenshot({path:'/tmp/audio-intel-unofficial-tooltip-desktop.png',fullPage:false})
 ...
 const englishFooterNotice=page.getByRole('button',{name:'UNOFFICIAL',exact:true})
 await englishFooterNotice.focus()
 await expect(identityTooltip).toBeVisible()
 await expect(identityTooltip).toContainText('has not been authorized, sponsored, approved, or endorsed')
 await page.getByLabel('Interface language').selectOption('zh-CN')
 await page.setViewportSize({width:390,height:844})
 await expect(page.locator('footer')).not.toBeVisible()
 const mobileIdentity=page.locator('.mobile-project-identity')
 await expect(mobileIdentity).toBeVisible()
 await expect(mobileIdentity).not.toHaveAttribute('open','')
 await expect(mobileIdentity.locator('p')).not.toBeVisible()
 const mobileSummary=mobileIdentity.locator('summary')
 const summaryBox=await mobileSummary.boundingBox()
 expect(summaryBox!.height).toBeGreaterThanOrEqual(44)
 await mobileSummary.click()
 await expect(mobileIdentity).toHaveAttribute('open','')
 const mobileDetail=mobileIdentity.locator('p')
 await expect(mobileDetail).toContainText('Sandevistan 名称及相关第三方知识产权的权利归各自权利人所有')
 await page.evaluate(()=>window.scrollTo(0,document.documentElement.scrollHeight))
 const detailBox=await mobileDetail.boundingBox()
 const navigationBox=await page.getByRole('navigation',{name:'主导航'}).boundingBox()
 expect(detailBox!.y+detailBox!.height).toBeLessThanOrEqual(navigationBox!.y)
 expect(await page.evaluate(()=>document.documentElement.scrollWidth)).toBeLessThanOrEqual(390)
 await expect(page.locator('vite-error-overlay')).toHaveCount(0)
 await page.screenshot({path:'/tmp/audio-intel-unofficial-details-mobile.png',fullPage:false})
 expect(errors).toEqual([])
})
...
test('ASR and TTS preferences persist independently and reset per page',async({page})=>{
 ...
 await expect(page.locator('.document-settings-toggle')).toContainText('GPU')
 ...
 await expect(page.locator('.document-settings-toggle')).toContainText('CPU')
...
test('first TTS submission appears immediately and survives a stale poll',async({page})=>{
 ...
 const queueItem=page.locator('.tts-recent-jobs .job-mini').filter({hasText:'首次合成即时入列'})
...
test('first ASR submission appears immediately from the accepted job response',async({page})=>{
 ...
 await page.getByRole('button',{name:'开始转写'}).click()
 await page.getByRole('button',{name:'查看本次任务'}).click()
 const queueItem=page.locator('.asr-recent-jobs .job-mini').filter({hasText:'first-submit.wav'})
 await expect(queueItem).toBeVisible({timeout:1000})
 await expect(queueItem).toContainText('等待处理')
 releaseList()
...
test('task history paginates on the server and pins an old selected result',async({page})=>{
 ...
 await expect(page).toHaveURL(/#asr$/)
 await expect(page.locator('.asr-recent-jobs .job-mini')).toHaveCount(6)
 await expect(page.locator('.asr-recent-jobs .job-mini[aria-current="true"]')).toContainText('历史任务 1')
 await page.locator('nav').getByRole('button',{name:/任务记录/}).click()
 await expect(page.getByText('第 3 / 3 页 · 共 61 条')).toBeVisible()
...
test('task lists stay newest-first and retain an older selected task',async({page})=>{
 ...
 await page.locator('nav').getByRole('button',{name:/转写工作台/}).click()
 await showAsrResults(page)
 await expect(page.locator('.asr-recent-jobs .job-mini')).toHaveCount(5)
 await expect(page.locator('.asr-recent-jobs .job-mini[aria-current="true"]')).toContainText('ASR 任务 7')
 await page.locator('nav').getByRole('button',{name:/任务记录/}).click()
 ...
 await expect(page.locator('.asr-recent-jobs .job-mini')).toHaveText([/ASR 任务 7/,/ASR 任务 6/,/ASR 任务 5/,/ASR 任务 4/,/ASR 任务 3/,/ASR 任务 1/])
 const selectedAsr=page.locator('.asr-recent-jobs .job-mini[aria-current="true"]')
 ...
 await expect(page).toHaveURL(/#tts$/)
 await expect(page.locator('.tts-task-list>h2')).toHaveText('最近合成任务')
 await expect(page.locator('.tts-result-detail>h2')).toHaveText('任务详情')
 ...
 await page.setViewportSize({width:390,height:844})
 await page.locator('.tts-task-picker select').scrollIntoViewIfNeeded()
 expect(await page.evaluate(()=>document.documentElement.scrollWidth)).toBeLessThanOrEqual(390)
 await expect(page.locator('.tts-task-picker select')).toBeInViewport()
 await expect(page.locator('.tts-task-picker option:checked')).toContainText('TTS 任务 1')
