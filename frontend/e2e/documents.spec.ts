...
test('upload retry reuses the request and failed parsing calls retry API; imports can be reused and removed',async({page})=>{
 ...
 await expect(row.getByRole('button',{name:'当前使用中'})).toBeDisabled()
 await page.getByRole('button',{name:'关闭已导入文档'}).click()
 await expect(page.locator('.document-sections li')).toHaveCount(2)
 await page.getByRole('button',{name:'已导入文档',exact:true}).click()
 await page.setViewportSize({width:390,height:844})
 expect(await page.evaluate(()=>document.documentElement.scrollWidth)).toBeLessThanOrEqual(390)
 ...
 await page.getByRole('dialog',{name:'移除所选文档',exact:true}).getByRole('button',{name:'移除所选文档',exact:true}).click()
 await expect(page.getByRole('dialog',{name:'移除所选文档',exact:true})).toHaveCount(0)
 await page.getByRole('dialog',{name:'已导入文档',exact:true}).getByRole('button',{name:'关闭已导入文档'}).click()
