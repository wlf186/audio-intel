...
export function HotwordsPage({items,state,limits,refresh}:{items:HotwordList[];state:ResourceState;limits?:HotwordLibraryCapability;refresh:()=>Promise<void>}){
 ...
 useEffect(()=>{if(name||terms||editingId)sessionStorage.setItem(draftKey,JSON.stringify({editingId,name,terms}));else sessionStorage.removeItem(draftKey)},[editingId,name,terms])
 ...
 const createLimitError=!editing&&limits&&items.length>=limits.max_lists?`最多只能创建 ${limits.max_lists} 个词表；请删除现有词表后再新建。`:''
 const invalidTerm=parsed.find(term=>term.length>maxTerm||/[\u0000-\u001f\u007f]/.test(term))
 const validationError=createLimitError||(!normalizedName||!parsed.length?'请输入场景名称和至少一个热词。':normalizedName.length>maxName?`场景名称不能超过 ${maxName} 个字符。`:invalidTerm?`热词“${invalidTerm.slice(0,18)}${invalidTerm.length>18?'…':''}”无效；单个热词不能超过 ${maxTerm} 个字符，也不能包含控制字符。`:parsed.length>maxTerms?`每个词表最多包含 ${maxTerms} 个热词。`:'')
