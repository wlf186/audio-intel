...
export function HotwordsPage({items,state,limits,refresh}:{items:HotwordList[];state:ResourceState;limits?:HotwordLibraryCapability;refresh:()=>Promise<void>}){
 ...
 const systemTermHelp=viewingShortSystem?'系统按保守规则自动提取去姓名字，无法可靠提取时跳过。':'自动同步已开启“加入热词库”的声纹人员完整姓名。'
 const systemNote=viewingShortSystem?'此词表由系统自动去姓生成，并非昵称，不能修改或删除。':'此词表由系统同步完整姓名，不能修改或删除。'
