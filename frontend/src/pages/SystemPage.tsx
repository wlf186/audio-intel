...
export function SystemPage({health,systemError,retry}:{health?:Health;systemError:string;retry:()=>Promise<void>}){
 ...
 return <section className="page-pad system hud-page" data-module="SYSTEM_CORE / SYS_04"><div className="page-heading"><div><h1 tabIndex={-1}>系统状态</h1><p>硬件资源、工作进程、模型与本地存储一览</p></div></div>{!health?<div className={`system-resource-state ${systemError?'error':'loading'}`} role={systemError?'alert':'status'}>{systemError?<RefreshCw/>:<LoaderCircle className="spin"/>}<h2>{systemError?'系统状态加载失败':'正在读取系统状态'}</h2><p>{systemError||'正在连接本地服务并读取硬件、模型和工作进程信息…'}</p>{systemError?<button className="button" onClick={()=>void retry()}>重新检查</button>:null}</div>:<><div className="resource-grid"><div className="resource-card"><small>CPU</small><strong>{Math.round(hardware?.cpu_percent||0)}%</strong><Meter value={hardware?.cpu_percent||0} label="CPU 使用率"/></div><div className="resource-card"><small>内存</small><strong>{gib(hardware?.memory_used)} / {gib(hardware?.memory_total)} GB</strong><Meter value={memory} label="系统内存"/></div><div className="resource-card"><small>GPU</small><strong>{gpu?`${gpu.memory_used_mib} / ${gpu.memory_total_mib} MiB`:'未检测到'}</strong>{gpu?.memory_free_mib!==undefined?<p className="gpu-memory-detail">当前可用 {gpu.memory_free_mib} MiB{gpu.memory_system_reserved_mib!==undefined?<span title="按总显存减去当前已用和空闲显存估算，通常对应驱动或固件保留，不提供进程归属"> · 系统保留估算 {gpu.memory_system_reserved_mib} MiB</span>:null}</p>:null}<Meter value={gpu?.memory_total_mib?(gpu.memory_used_mib/gpu.memory_total_mib*100):0} label={gpu?.name||'GPU 不可用'}/></div><div className="resource-card"><small>项目磁盘</small><strong>{gib(hardware?.disk_used)} / {gib(hardware?.disk_total)} GB</strong><Meter value={disk} label="项目磁盘"/></div></div><div className="system-columns"><section><h2>模型状态</h2>{health.models.length?<div className="model-groups"><ModelGroup label="ASR 与公共组件" models={asrModels}/><ModelGroup label="TTS" models={ttsModels}/></div>:<p className="muted">未配置模型</p>}</section><section><h2>工作进程</h2><div className="worker-list">{health.workers.length?health.workers.map(worker=><div key={worker.id}><i className={worker.state!=='stopped'?'installed':''}/><span><b>{worker.kind.toUpperCase()} Worker</b><small>{worker.current_job_id||'等待任务'} · {formatLocalDateTime(worker.heartbeat_at)}</small></span><em>{workerStateLabel(worker.state)}</em></div>):<p className="muted">当前未发现工作进程</p>}</div><h2>本地目录</h2><div className="paths">{Object.entries(health.storage).map(([key,value])=><div key={key}><b>{key}</b><code>{value}</code></div>)}</div></section></div></>}</section>
}
