...
export default function App(){
 ...
 useEffect(()=>{if(!authenticated||capabilities?.events?.sse!==true){setEventsConnected(false);return}const source=new EventSource(capabilities.events.global_url);const snapshot=(event:MessageEvent<string>)=>{try{const payload=JSON.parse(event.data) as {jobs?:Job[]};if(payload.jobs){mergeJobs(payload.jobs);setJobsReady(true)}setEventsConnected(true);setConnectionError('')}catch{setConnectionError('任务事件格式无效')}};source.addEventListener('snapshot',snapshot as EventListener);source.addEventListener('heartbeat',()=>setEventsConnected(true));source.onopen=()=>setEventsConnected(true);source.onerror=()=>setEventsConnected(false);return()=>{source.close();setEventsConnected(false)}},[authenticated,capabilities?.events?.sse,capabilities?.events?.global_url,mergeJobs])
 ...
 const logout=async()=>{await api.logout();setJobs([]);setJobsReady(false);setPinnedJobs({});setSelected({});setHealth(undefined);setSystemError('');setCapabilities(undefined);setVoiceprints([]);setVoiceprintsState('loading');setHotwordLists([]);setHotwordsState('loading');setAuth({required:true,authenticated:false})}
