...
- `/api/v1/health`、`/api/v1/system` 和 OpenAPI `info.version` 现在统一报告 release-aware 版本，不再长期固定为 `0.1.0`。正式 tag 返回纯发布号，tag 后源码构建使用 `0.1.5+3.g8adfe46` 一类 SemVer build metadata，存在已跟踪本地修改时追加 `.dirty`；字段类型和响应结构不变。
