...
## v0.1.13 workspaces, waveforms and documentation — 2026-09-12

The release separates ASR creation from results and provides fixed text/document/results tabs for TTS. Result selection, lazy details, file/draft lifetimes, modal recovery and playback pause are covered by interaction tests. Audio artifacts share bounded waveform preparation, versioned sidecars, historical backfill and purge-safe reader leases. The document guide retains its API workflow, configuration limits and executable examples alongside the new UI guidance.

- Final complete backend run: **287 passed, 6 skipped** (platform-specific). Documentation/version/OpenAPI checks passed **27/27**; coverage now guards the dedicated document guide's submission chain and resource configuration.
- **91 browser scenarios** passed across the complete run and a focused i18n follow-up. The old language test was adapted to open the new hotword dialog, close it before switching language, and reopen it to verify translated list names. Coverage includes all five external Office samples, ASR/TTS fixed tabs at 1440/1280/1024/720/390 px, stable task selection, lazy result loading, waveform seek/cache/retry and playback independent of waveform failures.
- The executable document CLI also completed a two-section Markdown job with `--waveform --output`, retrieved the waveform and streamed a valid ZIP containing both MP3 files.
- Frontend typecheck and production build, dependency-lock verification, frontend and Linux API dependency audits, and mock ASR/TTS smoke checks passed. The build retains Vite's non-failing advisory for the main bundle exceeding 500 kB.
- The README capture script now enters the results tab before waiting for transcript/audio content. Both locales' four screenshots and the social preview were regenerated from the isolated mock service at `http://127.0.0.1:20811`. Desktop and 390 px screenshots were inspected; navigation, meaningful content, console/page errors and overflow are checked by Playwright. Browser plugin not available; repository Playwright with local Chromium was used.
...
Release preflight also exposed a Windows locale bug in the offline album-tag rollback reader. A cp1252-default regression reproduced both explicit rollback and replacement-failure recovery errors on Linux; reading the UTF-8 journal explicitly fixes them while retaining strict task/checksum comparisons. Native Windows CI validates the final fix. Public document links are also checked for inclusion in Git, so ignored local files cannot mask missing release targets.
