...
The pinned Qwen3-TTS 0.6B Base/CustomVoice models do not support natural-language style or emotion instructions, dedicated speaking-rate or pitch parameters, or public sampling controls. Keep `/api/v1/capabilities.tts.controls` authoritative, reject non-empty legacy `instruct` and OpenAI-compatible `instructions` with `422`, and never silently ignore unsupported controls. Enabling any such control requires a compatible pinned model and package, updated API/UI capability handling and documentation, contract tests, and real-inference validation.
